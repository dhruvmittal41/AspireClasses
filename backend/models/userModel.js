...
const findById = async (id) => {
    const { rows } = await db.query('SELECT id, full_name, email_or_phone, school_name, dob, gender, mobile_number, city, state, country, profilestat FROM users WHERE id = $1', [id]);
    return rows[0];
};
const findAll = async () => {
    const { rows } = await db.query(
